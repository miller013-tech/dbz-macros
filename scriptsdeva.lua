...
-- Só funciona se estiver atacando
-- Hotkey F12 para ataque de 30s

local spellFast = "Burning Waves"            -- ataque rápido (1s)
local spellCanudo = "Super Namekian Final Flash" -- canudo
local spell30s = "Kasai no Arashi"          -- ataque de 30s
...
macro(100, "Combo Auto", function()
  if g_game.isAttacking() then
    if os.clock() - lastCast > 1.05 then -- delay ~1s entre magias
      if fastCount < 5 then
        say(spellFast)
...
onCreatureHealthPercentChange(function(creature, healthPercent)
    ...
    if (creature:isMonster() or creature:isPlayer()) and creature:getPosition() and pos() then
        if getDistanceBetween(pos(), creature:getPosition()) <= 5 then
            creature:setText("\n\n\n\n" .. healthPercent .. "%") -- agora mais abaixo
        else
            creature:clearText()
        end
    end
...
-- Cria storage específico para cada char
storage.areaSpell = storage.areaSpell or "Kai Blast" -- valor padrão

-- Macro principal
macro(100, "Área Editável", function()
    if g_game.isAttacking() then  -- só ataca se estiver atacando algo
        say(storage.areaSpell)
    end
end)

-- Cria TextEdit visível na aba do bot
...
macro(10, "Andar para o Norte", function()
  schedule(10, function() walk(0) end) -- 0 = NORTH (cima)
end)
